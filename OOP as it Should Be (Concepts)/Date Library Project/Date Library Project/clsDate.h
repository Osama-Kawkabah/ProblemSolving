#pragma warning(disable : 4996) //  ⁄ÿÌ·  Õ–Ì— „⁄Ì‰
#pragma once // ÷„«‰ ⁄œ„  ÷„Ì‰ «·„·› „—… √Œ—Ï ≈–«  „ «” œ⁄«¡Â „—«—« Ê ﬂ—«—«

#include<iostream> //  ÷„Ì‰ „ﬂ »… «·≈œŒ«· Ê«·≈Œ—«Ã
#include<string> //  ÷„Ì‰ „ﬂ »… «·”·«”·
#include "clsString.h" //  ÷„Ì‰ „·› —√” ¬Œ—

using namespace std; // «” Œœ«„ „”«Õ… «·√”„«¡ «·ﬁÌ«”Ì…

class clsDate // »œ¡  ⁄—Ì› «·›∆… clsDate
{

private:

	short _Day = 1; //  ⁄—Ì› „ €Ì— «·ÌÊ„ Ê ÂÌ∆ Â »«·ﬁÌ„… «·«› —«÷Ì…
	short _Month = 1; //  ⁄—Ì› „ €Ì— «·‘Â— Ê ÂÌ∆ Â »«·ﬁÌ„… «·«› —«÷Ì…
	short _Year = 1900; //  ⁄—Ì› „ €Ì— «·”‰… Ê ÂÌ∆ Â »«·ﬁÌ„… «·«› —«÷Ì…

public:

	clsDate() // »œ¡  ⁄—Ì› «·œ«·… «·»‰«∆Ì… «·«› —«÷Ì…
	{
		time_t t = time(0); // «·Õ’Ê· ⁄·Ï «·Êﬁ  «·Õ«·Ì
		tm* NewTime = localtime(&t); //  ÕÊÌ· «·Êﬁ  «·„Õ·Ì ≈·Ï ﬂ«∆‰ tm
		_Day = NewTime->tm_mday; //  ⁄ÌÌ‰ ﬁÌ„… «·ÌÊ„ „‰ ﬂ«∆‰ tm
		_Month = NewTime->tm_mon + 1; //  ⁄ÌÌ‰ ﬁÌ„… «·‘Â— „‰ ﬂ«∆‰ tm
		_Year = NewTime->tm_year + 1900; //  ⁄ÌÌ‰ ﬁÌ„… «·”‰… „‰ ﬂ«∆‰ tm
	}

	clsDate(string sDate) // »œ¡  ⁄—Ì› «·œ«·… «·»‰«∆Ì… «·„” ﬁ»·… ·”·”·…
	{

		vector <string> vDate; // ≈‰‘«¡ „ €Ì— · Œ“Ì‰ √Ã“«¡  «—ÌŒ «·”·”·…
		vDate = clsString::Split(sDate, "/"); //  ﬁ”Ì„ «·”·”·… ≈·Ï √Ã“«¡ »«” Œœ«„ «·ÊŸÌ›… Split ›Ì ›∆… clsString

		_Day = stoi(vDate[0]); //  ÕÊÌ· Ê ⁄ÌÌ‰ ﬁÌ„… «·ÌÊ„
		_Month = stoi(vDate[1]); //  ÕÊÌ· Ê ⁄ÌÌ‰ ﬁÌ„… «·‘Â—
		_Year = stoi(vDate[2]); //  ÕÊÌ· Ê ⁄ÌÌ‰ ﬁÌ„… «·”‰…

	}

	clsDate(short Day, short Month, short Year) // »œ¡  ⁄—Ì› «·œ«·… «·»‰«∆Ì… «·„” ﬁ»·… ··√—ﬁ«„
	{

		_Day = Day; //  ⁄ÌÌ‰ ﬁÌ„… «·ÌÊ„
		_Month = Month; //  ⁄ÌÌ‰ ﬁÌ„… «·‘Â—
		_Year = Year; //  ⁄ÌÌ‰ ﬁÌ„… «·”‰…

	}

	clsDate(short DateOrderInYear, short Year) // »œ¡  ⁄—Ì› «·œ«·… «·»‰«∆Ì… «·„” ﬁ»·… · — Ì» «·ÌÊ„ ›Ì «·”‰…
	{
		// Â–« ”Ì‰‘∆  «—ÌŒ« Õ”»  — Ì» «·ÌÊ„ ›Ì «·”‰…
		clsDate Date1 = GetDateFromDayOrderInYear(DateOrderInYear, Year); // «·Õ’Ê· ⁄·Ï  «—ÌŒ „‰  — Ì» «·ÌÊ„ ›Ì «·”‰…
		_Day = Date1.Day; //  ⁄ÌÌ‰ ﬁÌ„… «·ÌÊ„
		_Month = Date1.Month; //  ⁄ÌÌ‰ ﬁÌ„… «·‘Â—
		_Year = Date1.Year; //  ⁄ÌÌ‰ ﬁÌ„… «·”‰…
	}

	void SetDay(short Day) { // »œ¡  ⁄—Ì› «·œ«·… SetDay · ⁄ÌÌ‰ ﬁÌ„… «·ÌÊ„
		_Day = Day; //  ⁄ÌÌ‰ ﬁÌ„… «·ÌÊ„
	}

	short GetDay() { // »œ¡  ⁄—Ì› «·œ«·… GetDay ··Õ’Ê· ⁄·Ï ﬁÌ„… «·ÌÊ„
		return _Day; // ≈—Ã«⁄ ﬁÌ„… «·ÌÊ„
	}

	__declspec(property(get = GetDay, put = SetDay)) short Day; //  ⁄—Ì› Œ«’Ì… «·ÌÊ„ »«” Œœ«„ «·œÊ«· «·”«»ﬁ…
