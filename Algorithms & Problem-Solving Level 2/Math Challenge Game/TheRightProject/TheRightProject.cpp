...
int SimpleCalculator(
	int Number1, int Number2, enOperationType OpType)
{
	switch (OpType) {
	...
	case enOperationType::Div: return Number1 / Number2; default: return Number1 + Number2;
	}
}
...
stQuestion GenerateQuestion(enQuestionsLevel QuestionLevel, enOperationType OpType) {
	...
	stQuestion Question; if (QuestionLevel == enQuestionsLevel::Mix)
	{
	...
	Question.OperationType = OpType; switch (QuestionLevel) {

	case enQuestionsLevel::EasyLevel:
		Question.Number1 = RandomNumber(1, 10); Question.Number2 = RandomNumber(1, 10);
		Question.CorrectAnswer = SimpleCalculator(Question.Number1,
			Question.Number2, Question.OperationType);
		Question.QuestionLevel = QuestionLevel; return Question;
	case enQuestionsLevel::MedLevel:           Question.Number1 = RandomNumber(10, 50);
...
void CorrectTheQuestionAnswer(stQuizz& Quizz, short QuestionNumber)
{
	...
	{
		Quizz.QuestionList[QuestionNumber].AnswerResult = false; Quizz.NumberOfWrongAnswers++;
		cout << "Worng Answer :-( \n";
...
void AskAndCorrectQuestionListAnswers(stQuizz& Quizz)
{
	for (short QuestionNumber = 0;
		...
		QuestionNumber++) {
		PrintTheQuestion(Quizz, QuestionNumber); Quizz.QuestionList[QuestionNumber].PlayerAnswer = ReadQuestionAnswer();
...
void StartGame() {
	char PlayAgain = 'Y'; do { ResetScreen();   
	PlayMathGame(); 
